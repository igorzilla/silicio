...
abstract class BaseUser extends BaseObject  implements Persistent {
  ...
  const PEER = 'UserPeer';
	...
	/**
	 * @var        array Design[] Collection to store aggregation of Design objects.
	 */
	protected $collDesigns;

	/**
	 * @var        Criteria The criteria used to select the current contents of collDesigns.
	 */
	private $lastDesignCriteria = null;
	...
	/**
	 * Clears out the collDesigns collection (array).
	 *
	...
	/**
	 * Initializes the collDesigns collection (array).
	 *
	 ...
	 * @return     void
	 */
	public function initDesigns()
	{
		$this->collDesigns = array();
	}
	 ...
	 * related Designs from storage. If this User is new, it will return
		...
		if ($this->collDesigns === null) {
			if ($this->isNew()) {
			   $this->collDesigns = array();
				...
				$criteria->add(DesignPeer::OWNER, $this->username);

				DesignPeer::addSelectColumns($criteria);
				$this->collDesigns = DesignPeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				DesignPeer::addSelectColumns($criteria);
				if (!isset($this->lastDesignCriteria) || !$this->lastDesignCriteria->equals($criteria)) {
					$this->collDesigns = DesignPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastDesignCriteria = $criteria;
	...
	public function countDesigns(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collDesigns === null) {
			if ($this->isNew()) {
				...
				$count = DesignPeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	public function addDesign(Design $l)
	{
		...
		if (!in_array($l, $this->collDesigns, true)) { // only add it if the **same** object is not already associated
			array_push($this->collDesigns, $l);
			$l->setUser($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collDesigns) {
				foreach ((array) $this->collDesigns as $o) {
