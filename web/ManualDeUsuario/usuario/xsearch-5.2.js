...
function searchAll(keyword){
	...
	document.open()
	document.clear()
	document.write('<link rel="stylesheet" href="buscador.css">')
	AddBody()
	...
	document.write("<center><p><font size="+1+">Resultados <b>"+(display_start+1)+"-"+(displast)+"</b> de <b>"+sites+"</b> para <b>"+keyword+"</b> Tiempo de búsqueda <b>"+((timeB-timeA)/1000)+"</b> segundos.</p></center>")

	if (displast>sites && finds!=0) displast=sites+1


	if (finds==0) {
		document.write("<h3><font color=Yellow>No se encontró <b>''"+keyword+"''</b></font></h3>"+
						"<p>La palabra - <b>"+keyword+"</b> - no se encuentra en la base de datos.</p>"+
						"<LI>Verifique que haya escrito bien la palabra.</li>"+
						"<LI>Intente nuevamente utilizando otra palabra clave y presione Buscar.</li>"+
	...
	for (q=display_start; q<q3; q+=1) {
		if (results[q].val>0) {
			...
			if (x1>0 && x2>0) {
				...
				document.write("<table border=0 width=500><tr><td><span class='xresult'>"+records[rc].description+"</span></td></tr></table><br>") //<span class='xlocation'>"+tmp+"</span><br> viene despues de </table> y antes de <br> que termina esta línea
			}
			q2++
   		}
	}
...
function AddBody() {
	...
	templateBody()
	//Tamaño de la casilla. size = 20
	document.write("<form name='searchform' method='post' action='javascript:doSearch()'><table border='0' width='100%'><tr><td align='center'><font face='Arial, Helvetica, sans-serif' size='3'><input name='searchwords' type='text' size='20'>&nbsp;&nbsp;<a href='javascript:doSearch()'><br><br>"+ButtonCode+"</a></font></td></tr></table></form><hr size=1>")
